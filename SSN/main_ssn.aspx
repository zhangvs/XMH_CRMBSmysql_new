<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="main_ssn.aspx.cs" Inherits="SSN.main_ssn" %>
...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#tree1").ligerTree({
                url: 'DataSSN/SSN_Base.ashx?Action=getUserTree&rnd=' + Math.random(),
                idFieldName: 'id',
        ...
        function getsysinfo() {
            $.ajax({
                type: "GET",
                url: "DataSSN/SSN_Base.ashx", /* 注意后面的名字对应CS的方法名称 */
                ...
                success: function (result) {
                    ...
                    document.title = rows[0].sys_value + "SSN标准化服务系统";
                    //$("#logo").attr("src", rows[1].sys_value);
                    $("#logo").attr("src", "images/login/website_logo.jpg");
                }
            });
        }
        function getuserinfo() {
            $.getJSON("DataSSN/SSN_Base.ashx?Action=GetUserInfo&rnd=" + Math.random(), function (data, textStatus) {
                //alert(data);
                $("#Username").html(data.name);
                if (data.title)
                    $("#userheader").attr("src", "http://172.178.1.211:8087/images/upload/portrait/" + data.title);
                else
                    $("#userheader").attr("src", "http://172.178.1.211:8087/images/icons/function_icon_set/user_48.png");
            });
        }

        function getUser() {
            $.ajax({
                url: "DataSSN/SSN_Base.ashx?Action=GetOnline&rnd=" + Math.random(),
                type: 'get',
                success: function () {
                ...
                error: function () {
                    javascript: location.replace("login_ssn.aspx");
                }
            })
        ...
        function changepwd() {
            var dialog = $.ligerDialog.open({
                url: "person/hr_changepwd.aspx", width: 480, height: 250, title: "修改密码", buttons: [
                        {
        ...
        function toolbar() {
            $.getJSON("DataSSN/SSN_App.ashx?Action=GetSysApp&rnd=" + Math.random(), function (data, textStatus) {
        ...
        function f_according(appid) {
            ...
            mainmenu.empty();
            $.getJSON("DataSSN/SSN_Base.ashx?Action=GetSysApp&appid=" + appid + "&rnd=" + Math.random(), function (menus) {
        ...
        function show_welcome(item) {
            if (getCookie("xhd_crm_show_wellcome") == 1 || item == 1) {
                var dialog = $.ligerDialog.open({
                    url: "welcome.htm", width: 800, height: 500, title: "欢迎使用新明辉SSN系统", buttons: [
                            {
                                text: '关闭', onclick: function (item, dialog) {
                                    dialog.close();
                                }
        ...
        function personalinfoupdate() {
            var dialog = $.ligerDialog.open({
                url: "person/emp_personal_update.aspx", width: 760, height: 300, title: "个人信息", buttons: [
                        {
                            text: '保存', onclick: function (item, dialog) {
                                dialog.frame.f_save();
                            }
        ...
        <div id="layout1" style="width: 100%">
            ...
            <div position="center" id="framecenter">
                <div tabid="home" title="桌面" style="height: 300px">
                    <%--<iframe frameborder="0" name="home" id="mainframe" src="ssn/article/ssn_all_article.aspx"></iframe>--%>
                    <asp:ScriptManager ID="ScriptManager1" runat="server">
                    </asp:ScriptManager>
                    <asp:UpdatePanel ID="UpdatePanel1" runat="server" UpdateMode="Conditional" ChildrenAsTriggers="true">
                        <ContentTemplate>
                            <div id="toolbar1" class="l-toolbar">
                                ...
                                <div class="l-toolbar-item l-panel-btn l-toolbar-item-hasicon">
                                    <span><asp:Button ID="Button1" runat="server" Text="搜索" style="width: 60px; height: 18px; margin-left:-20px; background: url(../../images/search.gif) 1px 1px no-repeat;  margin-top:1px;"  OnClick="Button1_Click" BorderStyle="None"/></span>
                                    <%--<asp:Button ID="Button1" runat="server" Text="搜索" OnClick="Button1_Click" />--%>
                                    <%--<div class="l-panel-btn-l"></div>
                                    <div class="l-panel-btn-r"></div>
                                    <div class="l-icon" style="background: url(images/search.gif) 1px 1px no-repeat;"></div>--%>
                                </div>
                            </div>
                            ...
                            <div id="grid" style="margin-top: 30px;">
                                <ul class="tags">
                                    <asp:Repeater ID="repArticle" runat="server">
                                        <ItemTemplate>
                                            <li><a href='ssn/article/ssn_article_view.aspx?flag=<%#Eval("Id") %>' target="_blank"><%# Eval("Art_title")%></a></li>
                                        </ItemTemplate>
                                    </asp:Repeater>
                                </ul>
                            </div>
