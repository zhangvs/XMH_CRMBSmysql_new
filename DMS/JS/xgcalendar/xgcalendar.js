...
; (function ($) {
    ...
    if (!DateAdd || typeof (timestampdiff) != "function") {
    ...
    if (!timestampdiff || typeof (timestampdiff) != "function") {
        var timestampdiff = function (interval, d1, d2) {
            switch (interval) {
                case "d": //天
                case "w":
    ...
    $.fn.bcalendar = function (option) {
        ...
        function pushER(start, end) {
            ...
            if (ll == 0) {
                ...
                for (var i = 0; i < ll; i++) {
                    ...
                    var diff = timestampdiff("d", start, dr.startdate);
                    if (diff == 0 || diff == 1) {
                        if (dr.enddate < end) {
                            dr.enddate = end;
                        }
                        ...
                        var d2 = timestampdiff("d", end, dr.startdate);
                        if (d2 > 1) {
                        ...
                        var d3 = timestampdiff("d", end, dr.startdate);
                ...
                if (ll > 1) {
                    for (var i = 0; i < ll - 1;) {
                        ...
                        var diff1 = timestampdiff("d", d2.startdate, d1.enddate);
                        if (diff1 <= 1) {
            ...
            for (var i = 0; i < l; i++) { //处理全天和跨日的日程
                ...
                if (de.length > 0) { //有日程           
                    for (var j = 0; j < de.length; j++) {
                        var end = timestampdiff("d", lrdate, de[j].event[3]) > 0 ? lrdate : de[j].event[3];
                        de[j].colSpan = timestampdiff("d", dayarrs[i].date, end) + 1
                    }
                }
                de = null;
            }
        ...
        function formartEventsInHashtable(events, startday, daylength, rbdate, redate) {
            ...
            for (var i = 0; i < l; i++) {
                var sD = events[i][2];
                var eD = events[i][3];
                var diff = timestampdiff("d", sD, eD);
                var s = {};
                s.event = events[i];
                s.day = sD.getDate();
                s.year = sD.getFullYear();
                ...
                if (diff > 0) {
                    ...
                    for (; sdtemp <= eD; sD = sdtemp = DateAdd("d", daylength, sdtemp)) {
                        ...
                        var x = timestampdiff("d", sdtemp, eD);
                        if (hast[key] == null) {
                            hast[key] = [];
                        }
                        //处理表格跨界，分为跨周和不跨周
                        d.colSpan = (x >= daylength) ? daylength - timestampdiff("d", sdtemp, sD) : timestampdiff("d", sD, eD) + 1;
                        hast[key].push(d);
                        d = null;
                    }
                }
        ...
        function BuildMonthDayEvent(e, cday, length) {
            ...
            var ef = timestampdiff("d", cday, e.event[3]) >= length;  //e.event[3] >= DateAdd("d", 1, cday);
            if (sf || ef) {
                if (sf) {
                    arrm.push(ml);
        ...
        function dayshow(e, data) {
            ...
            if (data != null) {
                if (option.quickDeleteUrl != "" && data[8] == 1 && option.readonly != true) {
                    ...
                    var ss = [];
                    var iscos = timestampdiff("d", data[2], data[3]) != 0;
        ...
        function dragStart(type, e) {
            ...
            switch (type) {
                ...
                case "dw5":
                    ...
                    ya.push({ s: top, e: top + h });
                    var data = getdata(obj);
                    var dp = timestampdiff("d", data[2], data[3]) + 1;
                ...
                switch (d.type) {
                    ...
                    case 7:
                        if (d.lasso) {
                            ...
                            var diff = timestampdiff("d", start, currrentdate);
