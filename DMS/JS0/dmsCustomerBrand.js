...
var goods_numbers = new Array();//月销售数量
var goods_prices = new Array();//月销售单价
...
function getMonthSaleBackData(GridData, smonth) {
    ...
    if (GridData.Rows.length > 0) {
        ...
        for (var j = 0; j < GridData.Rows.length; j++) {
            ...
            if ($.inArray(brand_id, brand_ids) < 0) {
                ...
                goods_numbers[seriess_id] = new Array();//品牌下的单品id
                goods_prices[seriess_id] = new Array();//品牌下的单品名称
            ...
            goods_numbers[seriess_id][i] = GridData.Rows[j].goods_number;
            goods_prices[seriess_id][i] = GridData.Rows[j].goods_price;
            i++;
        }
    }
    //IE不支持forEach，兼容
    ...
    saleBackData.forEach(function DrawEChart(v, e) {
        ...
        var option = {
            ...
            legend: {
                data: ['月销售额', '月退货额', '月实际销售额', '月销售数量', '月销售单价'],
                selected: {
                    '月销售额': false,
                    '月退货额': false,
                    '月实际销售额': true,
