...
function getMonthSaleBackData(GridData) {
    ...
    if (GridData.Rows.length > 0) {
        if (nowYear > Number(syear)) {
            nowDays=360;//往年直接固定值
            workDays=300
        }
        else {
            datetime = GridData.Rows[0].datetime;
            nowDays = (nowMonth - parseInt(datetime) + 1) * 30;//今年到目前为止的天数
        ...
        for (var j = 0; j < GridData.Rows.length; j++) {
            ...
            year_count = GridData.Rows[j].year_count;
            if (year_count == null) {
                year_count = 0;
            }
            ...
            var ii = $.inArray(user_name, userNames);
            if (ii < 0) {
                ...
                if (year_count == 0) {
                    ...
                    year_counts.push(year_count);
                    year_frequencys.push(parseFloat(nowDays / year_count).toFixed());//客户第一次出现，拿货频率为该年频率
                }
            }
            else {
                if (realmoneyUsers[ii] != realmoneyUser) {
                    salemoneyUsers[ii] += salemoneyUser;
                    backmoneyUsers[ii] += backmoneyUser;
                    realmoneyUsers[ii] += realmoneyUser;
                    //客户已存在，拿货频率累加，拿货次数累加；
                    year_counts[ii] += year_count;
                    if (year_counts[ii] == 0) {
    ...
    //    if (realmoneyUsers[i]<1000) {
    //        userIds.splice(i, 1);
    //        uCompanys.splice(i, 1);
    //        userNames.splice(i, 1);
    //        year_counts.splice(i, 1);
    //        year_frequencys.splice(i, 1);
    //        salemoneyUsers.splice(i, 1);
    //        backmoneyUsers.splice(i, 1);
    //        realmoneyUsers.splice(i, 1);
    //        i--;
    //    }
    //}
    for (var i = 0; i < realmoneyUsers.length; i++) {
        ...
        toolbox: {
            ...
            feature: {
                ...
                myTool: {
                    show: true,
                    title: '导出excel',
                    icon: 'image://http://localhost:13121/Images/Icon/8.png',
                    onclick: function () {
                        loadDrugs();
                    }
                },
...
function loadDrugs() {
    var syear = $("#syear").val();
    window.open("/DMS/Reports_DMS.ashx?Action=getYearCustomer&syear=" + syear + "&admin_user_name=" + admin_user_name + "&rnd=" + Math.random());
    closePort();
}