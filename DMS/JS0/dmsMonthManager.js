...
function showMonthNodeData() {
    for (var i = 0; i < d_pids.length; i++) {
        ...
        if (nodeChaData[i][node_one - 1].value > 0) {
            getPoint(node_Point[i], '1节点奖', node_one - 1, nodeDataColor[i][node_one - 1][0]);
        }
        if (nodeChaData[i][node_two - 1].value > 0) {
            getPoint(node_Point[i], '2节点奖', node_two - 1, nodeDataColor[i][node_two - 1][0]);
        }
        if (nodeChaData[i][node_three - 1].value > 0) {
            getPoint(node_Point[i], '3节点奖', node_three - 1, nodeDataColor[i][node_three - 1][0]);
        }
        if (nodeChaData[i][node_four - 1].value > 0) {
            getPoint(node_Point[i], '4节点奖', node_four - 1, nodeDataColor[i][node_four - 1][0]);
        }
    }
