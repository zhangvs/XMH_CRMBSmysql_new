    ...
    for (var c = 0; c < cycles.length; c++) {
        ...
        if (c == 0) {
            for (var i = 0; i < obj.length; i++) {
                var e = singlecount - 1;
                getCompareData(c, parseFloat(wholeAdds[i][e].toFixed()), parseFloat(wholeChas[i][e].toFixed()), parseFloat(wholeWcls[i][e].toFixed()));//整
            ...
            for (var i = 0; i < obj.length; i++) {
                ...
                getCompareData(c, parseFloat(groupAdds[i][d].value.toFixed()), parseFloat(groupChas[i][d].value.toFixed()), parseFloat(groupWcls[i][d].value.toFixed()));//组
...
//月，节点，分部门综合展示
function getCompareData(c, sale, cha, wcl) {
    if (!isFinite(wcl)) {
        wcl = 0;
    }
