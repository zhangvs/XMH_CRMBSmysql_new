...
namespace XHD.DAL
{
	...
	public partial class ecs_month_baseDAO
	{
		...
		public XHD.Model.ecs_month_base GetModel(int plan_id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select plan_id,plan_year,plan_task,plan_jan,plan_feb,plan_mar,plan_apr,plan_may,plan_jun,plan_jul,plan_aug,plan_sep,plan_oct,plan_nov,plan_dec,kf_rname,kf_uname from ecs_month_base ");
			strSql.Append(" where plan_id=@plan_id");
			MySqlParameter[] parameters = {
					new MySqlParameter("@plan_id", MySqlDbType.Int32)
			};
