...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#maingrid4").ligerGrid({
                columns: [
                    ...
                    { display: '评比', name: 'plan_task', width: 100, align: 'right' },
                ...
                url: "../DMS_Set.ashx?Action=getSetRating&rnd=" + Math.random(),
        ...
        function doserch() {
            ...
            var sendtxt = "&Action=getSetRating&idlist=" + tree0 + "&syear=" + syear + "&rnd=" + Math.random();
        ...
        function Confirm() {
            ...
            var sendtxt = '';
            if (plan_id != '') {
                sendtxt = "&Action=updateSetRating";
            }
            else {
                sendtxt = "&Action=addSetRating";
            }
                ...
                $.ligerDialog.confirm("确定要删除吗？", function (yes) {
                    if (yes) {
                        $.ajax({
                            ...
                            data: { Action: "deleteSetRating", plan_id: row.plan_id, rnd: Math.random() },
                            success: function (responseText) {
                                if (responseText == "4") {
...
<body>
    <form id="form1" onsubmit="return false">
        ...
        <div class="dialog open" id="prodown" style="display: none; position: fixed; width:400px; z-index: 9999; background: #fff;">
            <div class="dialog-head" id="dhead1"><span class="close rotate-hover" style="cursor: pointer" onclick="closeDivs();">X</span><strong id="titlsAll">年评比设置</strong></div>
