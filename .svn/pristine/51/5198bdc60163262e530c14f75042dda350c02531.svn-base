...
namespace XHD.DAL
{
    ...
    public partial class scm_spe_order_goods
    {
        ...
        public int GetMaxId()
        {
            return DbHelperMySQL.GetMaxID("id", "scm_spe_order_goods");
        }
        ...
        public bool Exists(int id)
        {
            ...
            strSql.Append("select count(1) from scm_spe_order_goods");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into scm_spe_order_goods(");
            strSql.Append("id,order_id,order_goodsid,order_goodssn,goods_num,goods_price)");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update scm_spe_order_goods set ");
        ...
        public bool Delete(int id)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from scm_spe_order_goods ");
            strSql.Append(" where id=@id ");
            MySqlParameter[] parameters = {
					new MySqlParameter("@id", MySqlDbType.Int32,11)			};
            parameters[0].Value = id;
        ...
        public XHD.Model.scm_spe_order_goods GetModel(int id)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,order_id,order_goodsid,order_goodssn,goods_num,goods_price from scm_spe_order_goods ");
            strSql.Append(" where id=@id ");
            MySqlParameter[] parameters = {
					new MySqlParameter("@id", MySqlDbType.Int32,11)			};
            parameters[0].Value = id;

            XHD.Model.scm_spe_order_goods model = new XHD.Model.scm_spe_order_goods();
        ...
        public XHD.Model.scm_spe_order_goods GetModel(string strWhere)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,order_id,order_goodsid,order_goodssn,goods_num,goods_price ");
            strSql.Append(" FROM scm_spe_order_goods ");
            if (strWhere.Trim() != "")
            {
        ...
        /// 得到一个对象实体
        /// </summary>
        public XHD.Model.scm_spe_order_goods DataRowToModel(DataRow row)
        {
            XHD.Model.scm_spe_order_goods model = new XHD.Model.scm_spe_order_goods();
            if (row != null)
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select sog.id,sog.order_id,sog.order_goodsid,sog.order_goodssn,sog.goods_num,sog.goods_price,g.goods_name ");
            strSql.Append(" FROM scm_spe_order_goods sog left join ecs_goods g on sog.order_goodsid=g.goods_id");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        {
            string strSql = @"INSERT INTO scm_spe_order_goods(order_id,order_goodsid,order_goodssn,goods_num,goods_price)
                            SELECT spe.id,og.order_goodsid,og.order_goodssn,og.goods_num,og.goods_price FROM scm_spe_goods og 
                            INNER JOIN (
                            SELECT od.id,so.order_id_old,so.id sid  FROM scm_spe_order so 
                            INNER JOIN scm_spe_order_detail od
                            ON so.id = od.spe_order_id
                            AND od.id NOT IN (
                            		SELECT order_id FROM scm_spe_order_goods
                            	)
                            AND od.order_suppliersid={0}) spe ON og.spe_order_id=spe.sid 
                            INNER JOIN ecs_goods g ON g.goods_id=og.order_goodsid AND g.suppliers_id={1}";
        ...
        public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
        {
            ...
            strSql.Append(")AS Row, T.*  from scm_spe_order_goods T ");
        ...
        public DataSet GetList(int PageSize,int PageIndex,string strWhere)
        {
            ...
            parameters[0].Value = "scm_spe_order_goods";
