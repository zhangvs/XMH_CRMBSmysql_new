		...
		public List<XHD.Model.Personal_Calendar> DataTableToList(DataTable dt)
		{
			...
			if (rowsCount > 0)
			{
				...
				for (int n = 0; n < rowsCount; n++)
				{
					...
					{
                        DateTime sdatetime = Convert.ToDateTime(Convert.ToDateTime(dt.Rows[n]["StartTime"]).ToUniversalTime());
                        String sdate = sdatetime.ToString("yyyy/MM/dd HH:mm:ss").Replace('-', '/');
                        model.StartTime = DateTime.Parse(sdate);
					}
					if(dt.Rows[n]["EndTime"].ToString()!="")
					{
                        DateTime edatetime = Convert.ToDateTime(Convert.ToDateTime(dt.Rows[n]["EndTime"]).ToUniversalTime());
                        String edate = edatetime.ToString("yyyy/MM/dd HH:mm:ss").Replace('-', '/');
                        model.EndTime = DateTime.Parse(edate);
					}
					...
					{
                        DateTime UPdatetime = Convert.ToDateTime(Convert.ToDateTime(dt.Rows[n]["UPTime"]).ToUniversalTime());
                        String UPdate = UPdatetime.ToString("yyyy/MM/dd HH:mm:ss").Replace('-', '/');
                        model.UPTime = DateTime.Parse(UPdate);
					}
