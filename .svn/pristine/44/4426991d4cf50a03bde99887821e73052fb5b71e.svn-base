<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="art_view.aspx.cs" Inherits="SSN.SSN.art_view" %>
        ...
        $(function () {
            ...
            if (getparastr("flag")) {
                $.ajax({
                    ...
                    data: { Action: 'form', flag: getparastr("flag"), title: "", rnd: Math.random() }, /* 注意参数的格式和名称 */
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function (result) {
                        var imgdiv = "";
                        if (result.image_lst) {
                            var urlList = result.image_lst;
                            var myarray = urlList.split(',');
                            for (var n in myarray) {
                                if (myarray[n]) {
                                    imgdiv += "        <img style='width: 200px;' alt='' src='../" + myarray[n] + "' />";
                                }
                            }
                        ...
                        centreview.html('');
                        var html = "<div style='width: auto;'>"
                                    + "<div class='title_art'>" + result.Art_title + "</div>"
                                    //+ "<div class='content_art'>" + result.Art_title + "</div>"
                                    + "<div  id='testDiv' class='content_art' style='overflow: hidden; padding: 10px;'>"
                                    + result.Art_Content.replace("�", "").replace(" �", "")
                                    + imgdiv
                                    + "</div>"
                                    + "<div id='enlarge_images'></div>"
                                    + "</div>";
                        centreview.html(html);
                        afterImg();
                    }
                });
            }
        ...
        function load() {
            ...
            $.getJSON("../DataSSN/SSN_NewPage.ashx?Action=GetSysMenu1&appid=1&rnd=" + Math.random(), function (menus) {
                if (menus.Total == '0') {
                    ...
                    //$(".tc").show();
                    //$("#out").hide();
                    javascript: location.replace("main.aspx");
                } else {
                    $(".tc").hide();
    ...
    function afterImg() {
        ...
        for (i = 0; i < gg.length; i++) {
            ...
            ts.onmousemove = function (event) {
                ...
                ei.innerHTML = '<img height="500px" weight="500px" src="' + this.src + '" />';
                ei.style.top = document.body.scrollTop + event.clientY - 300 + "px";
