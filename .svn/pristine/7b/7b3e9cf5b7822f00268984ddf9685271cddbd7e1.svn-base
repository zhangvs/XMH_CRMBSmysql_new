...
namespace FMS_DAL
{
    ...
    public partial class shop_invoices
    {
        ...
        public int GetMaxId()
        {
            return Shop_DbHelperMySQL.GetMaxID("invoice_id", "shop_invoices");
        }
        public DataSet GetParams(string sql)
        {
            return Shop_DbHelperMySQL.Query(sql);
        }

        public bool Update(string sql)
        {
            int rows = Shop_DbHelperMySQL.ExecuteSql(sql);
            if (rows > 0)
            {
                return true;
            }
        ...
        public bool Exists(int invoice_id)
        {
            ...
            strSql.Append("select count(1) from shop_invoices");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from shop_invoices ");
            strSql.Append(" where invoice_id=@invoice_id ");
            MySqlParameter[] parameters = {
					new MySqlParameter("@invoice_id", MySqlDbType.Int32,11)			};
            parameters[0].Value = invoice_id;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(" select ca.C_name, ca.C_tel,c.id as customerid, c.Customer,c.address as site,c.Nation,c.Nation_id,c.Provinces_id, ");
            strSql.Append(" c.Provinces,c.City,c.City_id,e.name,e.emp_code,d.dept_code_kingdee, i.*  from xmh_shop.shop_invoices i ");
            strSql.Append(" LEFT JOIN hr_employee e on e.ID=i.admin_id ");
            strSql.Append(" LEFT JOIN hr_department d on d.id=e.d_id ");
            strSql.Append(" LEFT JOIN crm_contact ca on i.user_id=ca.id ");
            strSql.Append(" LEFT JOIN crm_customer c on ca.C_customerid=c.id ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        public DataSet GetRequestBill(int PageSize, int PageIndex, string strWhere, string filedOrder, out string Total)
        {
            ...
            strSql.Append(" select i.* ,e.name,CONCAT(c.C_name,',',c.C_tel,',',i.company_name) as customer,c.C_name,c.C_tel,s.Customer,s.privatecustomer,s.BeSeats_id,s.BeSeats_uid,s.BeSeats  ");
            strSql.Append(" FROM  xmh_shop.shop_invoices  i ");
            strSql.Append(" inner join hr_employee e  on i.admin_id=e.ID ");
            strSql.Append(" inner join crm_contact c on c.id=i.user_id ");
            strSql.Append(" inner join crm_customer s on s.id=c.C_customerid ");

            strSql1.Append(" select count(i.invoice_id)  FROM  xmh_shop.shop_invoices   i ");
            strSql1.Append(" inner join hr_employee e  on i.admin_id=e.ID ");
            strSql1.Append(" inner join crm_contact c on c.id=i.user_id ");
            strSql1.Append(" inner join crm_customer s on s.id=c.C_customerid ");
        ...
        public DataSet GetList(int PageSize, int PageIndex, int mark, string strWhere, DataTable dt, string filedOrder, out string Total)
        {
            ...
            if (mark==1)
            {
                ...
                strSql1.Append(" select count(distinct invoice_id) from ( ");
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        if (i == 0)
                        {
                            ...
                            strSql.Append(" where  i.invoice_status in (5,7,10,11,14,15,16,17) and i.user_id  ='" + dt.Rows[i]["userId"].ToString() + "' and i.add_time>='" + dt.Rows[i]["datetime"].ToString() + "' ");
                            strSql1.Append(" select i.*   FROM  xmh_shop.shop_invoices  i ");
                            strSql1.Append(" where  i.invoice_status in (5,7,10,11,14,15,16,17) and i.user_id  ='" + dt.Rows[i]["userId"].ToString() + "' and i.add_time>='" + dt.Rows[i]["datetime"].ToString() + "' ");
                        }
                        else
                        {
                            strSql.Append(" union all select i.* ,e.name,CONCAT(c.C_name,',',c. C_tel,',',s.Customer) as customer,s.privatecustomer,s.BeSeats_id  FROM  xmh_shop.shop_invoices  i ");
                            strSql.Append(" left join hr_employee e  on i.admin_id=e.ID ");
                            strSql.Append(" left join crm_contact c on c.id=i.user_id ");
                            strSql.Append(" left join crm_customer s on s.id=c.C_customerid ");
                            strSql.Append(" where  i.invoice_status in (5,7,10,11,14,15,16,17) and i.user_id  ='" + dt.Rows[i]["userId"].ToString() + "' and i.add_time>='" + dt.Rows[i]["datetime"].ToString() + "' ");
                            strSql1.Append(" union all select i.*   FROM  xmh_shop.shop_invoices  i ");
                            strSql1.Append(" where  i.invoice_status in (5,7,10,11,14,15,16,17) and i.user_id  ='" + dt.Rows[i]["userId"].ToString() + "' and i.add_time>='" + dt.Rows[i]["datetime"].ToString() + "' ");
                        }
                    }
                }
        ...
        public DataSet GetUserBillInfo(string strWhere, string strWhereOne)
        {
            ...
            strSql.Append(" re.address as billaddress,re.zipcode,re.tel as re_tel,re.mobile,re.is_invoice,e.name,c.C_name,c.C_tel,s.Customer,s.privatecustomer,s.BeSeats_id   FROM  xmh_shop.shop_invoices   i ");
            strSql.Append(" left join hr_employee e  on i.admin_id=e.ID ");
            strSql.Append(" left join crm_contact c on c.id=i.user_id ");
            strSql.Append(" left join crm_customer s on s.id=c.C_customerid ");
            //strSql.Append(" left join xmh_shop.shop_user_address d on d.user_id=c.id ");
            strSql.Append(" left join (select * from xmh_shop.shop_user_address  ad   where ad.user_id=( select user_id from xmh_shop.shop_invoices where  invoice_id=" + strWhereOne + " ) and ad.is_invoice='1') as re  on re.user_id=c.id ");
            strSql.Append(" left join (select region_name  as country_name,region_id from xmh_shop.shop_region r left join xmh_shop.shop_user_address  a on a.country=r.region_id where a.user_id=( select user_id from xmh_shop.shop_invoices where invoice_id=" + strWhereOne + " )  and a.is_invoice='1') as cou   on cou.region_id=re.country ");
            strSql.Append(" left join (select region_name as province_name,region_id from xmh_shop.shop_region r left join xmh_shop.shop_user_address  a on a.province=r.region_id  where a.user_id=( select user_id from xmh_shop.shop_invoices where invoice_id=" + strWhereOne + " ) and a.is_invoice='1') as pro  on pro.region_id=re.province");
            strSql.Append(" left join (select region_name  as city_name,region_id from xmh_shop.shop_region r left join xmh_shop.shop_user_address  a on a.city=r.region_id where a.user_id=( select user_id from xmh_shop.shop_invoices where invoice_id=" + strWhereOne + " )   and a.is_invoice='1') as ct on ct.region_id =re.city ");
            strSql.Append(" left join (select region_name  as district_name,region_id from xmh_shop.shop_region r left join xmh_shop.shop_user_address  a on a.district=r.region_id where a.user_id=( select user_id from xmh_shop.shop_invoices where invoice_id=" + strWhereOne + " )   and a.is_invoice='1') as dis  on dis.region_id=re.district ");
        ...
        {
            string strSql = " update shop_invoices set invoice_status='" + status + "',erp_order_sn='" + erpCode + "',invoice_time='" + time + "',update_time='" + time + "' where invoice_id in (" + invoice_id + ")";
            int rows = Shop_DbHelperMySQL.ExecuteSql(strSql.ToString());
            if (rows > 0)
            {
                return true;
        ...
        {
            string strSql = " update shop_invoices set invoice_status='" + status + "',update_time='" + time + "',invoice_time='" + time + "' where invoice_id in(" + invoice_id + ")";
            int rows = Shop_DbHelperMySQL.ExecuteSql(strSql.ToString());
            if (rows > 0)
            {
                return true;
