...
namespace XHD.DAL
{
    ...
    public partial class product
    {
        ...
        public DataSet getOrderNumForNewShop(int PageSize, int PageIndex, string strWhere, string filedOrder, out string Total)
        {
            ...
            StringBuilder strSql1 = new StringBuilder();
            strSql.Append(" select  g.goods_name,g.goods_id,g.brand_id,dh.rq,dh.bm,dh.djhm, ");
            strSql.Append("          dh.bzdw,sum(dh.sl) as sl,sum(dh.ydsl) as ydsl,sum(dh.wdsl) as wdsl,sum(dh.ztsl) as sjztsl,sum(dh.ztsl)+sum(dh.ydsl) as ztdjsl,dh.dhrq,dh.jl,     ");
            strSql.Append("         ifnull(dq.sl,0) as real_num, ");
            strSql.Append("         (ifnull(dq.sl,0) + ifnull(h.sj_ztsl,0) -ifnull(k.ddsl,0)) as purchase_can_sale, ");
            strSql.Append("         (ifnull(k.ddsl,0) - ifnull(k.zhsl,0)) as waiting_number, ");
            strSql.Append("         ifnull(k.zhsl,0) as locked_number ");
            strSql.Append(" from xmh_wms.dhdjk as dh ");
            strSql.Append(" left join xmh_shop.shop_goods as g on dh.bm=g.goods_id  ");
            strSql.Append(" left join (select bm, sum(sjztsl) as sj_ztsl from xmh_wms.dhdjk where is_close = 0 group by bm) h on h.bm=dh.bm ");
            strSql.Append(" left join (select goods_id, sum(goods_num) as ddsl,sum(locked_num) as zhsl from xmh_shop.shop_locked_goods group by goods_id ) k on dh.bm=k.goods_id ");
            strSql.Append("  inner join xmh_wms.dhxlk as x on dh.djhm=x.djhm  ");
            strSql.Append(" left join xmh_wms.dqkcyl as dq on dq.bm=dh.bm   and dq.hw='k1' ");

            strSql1.Append("  select count(1)  from (");
            strSql1.Append(" select count(dh.bm) FROM xmh_wms.dhdjk as dh left join xmh_shop.shop_goods as g on dh.bm=g.goods_id");
            strSql1.Append("  inner join xmh_wms.dhxlk as x on dh.djhm=x.djhm ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
                strSql1.Append(" where " + strWhere);
        ...
        public DataSet getOrderDetail(int PageSize, int PageIndex, string strWhere, string filedOrder, out string Total)
        {
            ...
            strSql.Append("  from xmh_wms.zt_djk as d");
            strSql.Append("  inner join xmh_wms.zt_xlk as x on d.djhm=x.djhm and x.jz='√'  ");
            strSql.Append("  inner join (");
            strSql.Append("  select distinct d.djhm from xmh_wms.dhdjk as d");
            strSql.Append("   inner join xmh_wms.dhxlk as x on d.djhm=x.djhm and  x.jz='√' ");
        ...
        public DataSet getOrderDetailForNewShop(int PageSize, int PageIndex, string strWhere, string filedOrder, out string Total)
        {
            ...
            strSql1.Append(" select count(d.DJHM) FROM xmh_wms.zt_djk as d");
            strSql1.Append("  inner join xmh_wms.zt_xlk as x on d.djhm=x.djhm ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
                strSql1.Append(" where " + strWhere);
