    ...
    public partial class ecs_suppliers
    {
        ...
        public DataSet GetList(string strWhere)
        {
            ...
            strSql.Append(@"select s.suppliers_id,s.scm_factoryId,s.suppliers_erp_code,s.suppliers_name,s.suppliers_tel,s.add_time,s.add_empId,s.add_empName,s.is_enable,s.wms_suppliers_id,s.Provinces_id,s.Provinces,s.City_id,s.City,s.District,s.District_id,s.suppliers_desc,
                            s.update_time,s.update_empId,s.update_empName,s.factory_UserNum,u.user_uid,u.Id,s.operate_brands,s.sync_outKingdee
        ...
        {
            MySqlParameter[] parameters = {
                    new MySqlParameter("@num",MySqlDbType.String,25)
					};
            parameters[0].Direction = ParameterDirection.Output;

            string strRe = DbHelperMySQL.RunProcedure("GetSupplyErpCode", parameters);
        ...
        //获取分页数据
        public DataSet GetPageData(int pageSize, int pageIndex, string tableName, string where_str, out string Total)
        {
            StringBuilder strSql = new StringBuilder();
            StringBuilder strSql1 = new StringBuilder();
            strSql.Append("Select *  From " + tableName);
            strSql1.Append(" select count(*) From  ecs_suppliers  " + where_str);

            strSql.Append(" order by suppliers_id  limit " + (pageIndex - 1) * pageSize + "," + pageSize);
            Total = DbHelperMySQL.Query(strSql1.ToString()).Tables[0].Rows[0][0].ToString();
            return DbHelperMySQL.Query(strSql.ToString());
