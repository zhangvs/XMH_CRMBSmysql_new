...
namespace XHD.BLL
{
	...
	{
		private readonly XHD.DAL.shop_purchase_goods dal=new XHD.DAL.shop_purchase_goods();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public XHD.Model.shop_purchase_goods GetModelByCache(int gid)
		{
			
			string CacheKey = "shop_purchase_goodsModel-" + gid;
			...
			return (XHD.Model.shop_purchase_goods)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<XHD.Model.shop_purchase_goods> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<XHD.Model.shop_purchase_goods> DataTableToList(DataTable dt)
		{
			List<XHD.Model.shop_purchase_goods> modelList = new List<XHD.Model.shop_purchase_goods>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				XHD.Model.shop_purchase_goods model;
